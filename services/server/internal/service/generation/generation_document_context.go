...
func (workflow *GenerationService) applyGenerationDocumentContext(payload *generationMessageRequest) error {
	...
	if context == nil {
		...
		// MCP generation requests can identify their source either with the
		// nested documentContext object or the top-level documentId/sectionId
		// fields. The Agent normally uses the latter. Normalize it here so a
		// resource image generated by the Agent inherits its document category
		// (character, scene, or prop) and can subsequently be used by an
		// existing storyboard mention.
		context = &GenerationDocumentContext{
			ProjectID:  strings.TrimSpace(payload.ProjectID),
			DocumentID: documentID,
			SectionID:  strings.TrimSpace(payload.SectionID),
		}
		payload.DocumentContext = context
	}
